...
import { useResidentTimeline } from "../../state/careEventsState";
...
export function AgencyResidentProfilePage() {
  ...
  // Called unconditionally (rules of hooks) even if `resident` turns out to
  // be undefined below -- an empty/unknown id just yields no live events.
  const timelineEvents = useResidentTimeline(residentId ?? "");
      ...
      <div style={{ padding: "0 var(--space-6)" }}>
        ...
        {tab === "Timeline" &&
          (timelineEvents.length > 0 ? (
            ...
            <Card>
              <p style={{ margin: 0, color: "var(--color-ink-soft)" }}>
                Todavía no hay actividad registrada para {resident.name.split(" ")[0]}.
              </p>
            </Card>
          ))}
