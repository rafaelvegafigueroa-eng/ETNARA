...
import { useShift, CURRENT_SHIFT_ID } from "./shiftState";
...
export function CaregiverShiftCompletePage() {
  const navigate = useNavigate();
  const { reset } = useShift();
  const { eventsForShift } = useCareEvents();
  const loggedCount = eventsForShift(CURRENT_SHIFT_ID).length;
  ...
  return (
    <div style={{ padding: "var(--space-8) var(--space-5)", textAlign: "center" }}>
      ...
      <p style={{ color: "var(--color-ink-soft)", marginBottom: "var(--space-6)" }}>
        La familia de Carmen ya puede verlo. Registraste {loggedCount}{" "}
        {loggedCount === 1 ? "evento" : "eventos"} durante el turno.
      </p>
      <PrimaryButton
        onClick={() => {
          reset();
