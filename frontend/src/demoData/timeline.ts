...
export interface DemoTimelineEvent {
  ...
  worker?: string;
  hasPhoto?: boolean;
  /** Actual image data for photo events registered live during this
   * session (base64 data URL). Absent for the pre-scripted demo photo
   * event, which keeps the original placeholder box. */
  photoSrc?: string;
  /** Minutes-since-midnight, used only to merge/sort live CareEvents
   * alongside this static list -- see services/careEvents/timelineAdapter.ts.
   * Optional and unused by the static demo entries themselves. */
